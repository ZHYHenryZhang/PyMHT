...
class Car():
  ...
  def __init__(self, position=[0,0,0], orientation=0, v = 0, a = 0, mode='constant acceleration'):
    ...
    self.b_static = False
    self.id = Car.count
    self.trajectory = [[self.pos[0]], [self.pos[1]]]
    Car.count += 1
  ...
  def simulate(self, dt):
    ...
    self.trajectory[0].append(self.pos[0])
    self.trajectory[1].append(self.pos[1])
  ...
  def get_object(self, pos = [0.0,0.0,0.0]):
  ...
  def viz(self):
    visual_mode = "particle"
    if visual_mode == "particle":
      # do some thing 
